...
class OriginalRUN(Optimizer):
    ...
    def evolve(self, epoch):
        ...
        for idx in range(0, self.pop_size):
            ...
            gama = self.generator.random() * (
                    self.pop[idx].solution - self.generator.uniform(0, 1, self.problem.n_dims) *
            ...
            if self.generator.random() < 0.5:
                w = self.uniform_random__(0, 2, self.problem.n_dims) * np.exp(
                    -5 * self.generator.random() * epoch / self.epoch)  # Eq.19-1
                ...
                x_new2_temp2 = x_new1 - x_ave + r * w * np.abs(
                    self.generator.normal(0, 1, self.problem.n_dims) + u * x_new1 - x_ave)
                ...
                if self.compare_target(tar_new2, self.pop[idx].target, self.problem.minmax):
                    ...
                    if w[self.generator.integers(0, self.problem.n_dims)] > self.generator.random():
                        ...
                        x_new3 = pos_new2 - self.generator.random() * pos_new2 + \
                                 SF[idx] * (SM + (2 * self.generator.random(
                            self.problem.n_dims) * self.g_best.solution - pos_new2))  # Eq. 20
