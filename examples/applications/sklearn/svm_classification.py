...
class SvmOptimizedProblem(Problem):
    ...
    def obj_func(self, x):
        ...
        y_predict = svc.predict(X_test_std)
        # Measure the performance
        return metrics.accuracy_score(y_test, y_predict), 2


data = [X_train_std, X_test_std, y_train, y_test]
my_bounds = [
    FloatVar(lb=0.01, ub=1000., name="C_paras"),
    StringVar(valid_sets=('linear', 'poly', 'rbf', 'sigmoid', None), name="kernel_paras")
]
problem = SvmOptimizedProblem(bounds=my_bounds, minmax="max", data=data, name="SVM class")

model = SHADE.OriginalSHADE(epoch=10, pop_size=50)
model = BBO.OriginalBBO(epoch=10, pop_size=20)
# print(model)

print(problem.n_dims)
print(problem.n_objs)
print(problem.obj_weights)
print(problem.name)

model.solve(problem)

print(model.problem.n_dims)
print(model.problem.n_objs)
print(model.problem.obj_weights)
print(model.problem.name)

print(f"Number of evaluations: {model.nfe_counter}")
